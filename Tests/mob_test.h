...
TEST(MobTest, DrakeLv1) {
  ...
  EXPECT_TRUE(drake1->get_mob_exp() > 1); //random
...
TEST(MobTest, TrollLv1) {
  ...
  EXPECT_NEAR(troll1->get_mob_exp(), 5, 60); //random
...
TEST(MobTest, WolfLv1) {
  ...
  EXPECT_NEAR(wolf1->get_mob_exp(), 20, 150); //random
...
TEST(MobTest, YetiLv1) {
  ...
  EXPECT_NEAR(yeti1->get_mob_exp(), 10, 80); //random
...
TEST(MobTest, DrakeLv5) {
  ...
  EXPECT_NEAR(drake1->get_mob_exp(), 15, 110); //random
  EXPECT_EQ(drake1->getName(), "DRAKE");
...
TEST(MobTest, TrollLv5) {
  ...
  EXPECT_NEAR(troll1->get_mob_exp(), 5, 70); //random
  EXPECT_EQ(troll1->getName(), "TROLL");
...
TEST(MobTest, YetiLv5) {
  ...
  EXPECT_NEAR(yeti1->get_mob_exp(), 50, 150); //random 
  EXPECT_EQ(yeti1->getName(), "YETI");
